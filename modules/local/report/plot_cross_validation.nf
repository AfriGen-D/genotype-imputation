process PLOT_CROSS_VALIDATION {
    ...
    input:
    tuple val(meta), val(ref_name), path(cv_results), val(chr)
    
    output:
    tuple val(meta), val(ref_name), path("*_cross_validation.pdf"), emit: plot
    ...
    def plot_out = "${prefix}_${ref_name}${chr_suffix}_cross_validation.pdf"
    def chr_arg = chr ? "--chr ${chr}" : ""
    def cv_arg = cv_results.name != 'NO_FILE' ? "--cv-results ${cv_results}" : ""
    """
    # Copy the Python script from bin directory
    cp ${projectDir}/bin/plot_cross_validation.py .
    
    # Run the script with appropriate arguments
    python3 plot_cross_validation.py \\
        ${plot_out} \\
        --sample-id "${meta.id}" \\
        --ref-name "${ref_name}" \\
        ${chr_arg} \\
    ...
    def chr_suffix = chr ? "_chr${chr}" : "_genome"
    """
    touch ${prefix}_${ref_name}${chr_suffix}_cross_validation.pdf
